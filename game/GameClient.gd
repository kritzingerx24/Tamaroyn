...
# Camera zoom (Insert\/Delete in original Wulfram).
var _base_camera_fov_deg: float = 70.0
var _camera_zoom_x: float = 1.0

# Radar range (F3 in original Wulfram): short\/long toggle.
var _radar_long: bool = false
...
var _queued_build_skypump: bool = false
...
func _ready() -> void:
	...
	_base_camera_fov_deg = camera.fov
	_camera_zoom_x = 1.0
	if is_instance_valid(hud) and hud.has_method("set_camera_zoom"):
		hud.call("set_camera_zoom", _camera_zoom_x)
...
func _input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed and not event.echo:
		...
		if event.keycode >= KEY_KP_0 and event.keycode <= KEY_KP_9:
			...
			return
		# View zoom (Wulfram): Insert\/Delete, shift variants jump to max\/min.
		if event.keycode == KEY_INSERT:
			if event.shift_pressed:
				_set_camera_zoom(28.6)
			else:
				_set_camera_zoom(_camera_zoom_x * 1.25)
			return
...
func _set_camera_zoom(zoom_x: float) -> void:
	_camera_zoom_x = clamp(zoom_x, 1.0, 28.6)
	if is_instance_valid(camera):
		var new_fov: float = _base_camera_fov_deg / max(1.0, _camera_zoom_x)
		# Keep fov reasonable (avoid 0/inf).
		camera.fov = clamp(new_fov, 2.0, _base_camera_fov_deg)
	if is_instance_valid(hud) and hud.has_method("set_camera_zoom"):
		hud.call("set_camera_zoom", _camera_zoom_x)
...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("toggle_radar_range"):
		_radar_long = not _radar_long
		var rr: float = 280.0 if _radar_long else 140.0
		if is_instance_valid(hud) and hud.has_method("set_radar_range_m"):
			hud.call("set_radar_range_m", rr)
			if hud.has_method("flash_message"):
				hud.call("flash_message", "Radar: %s" % ("LONG" if _radar_long else "SHORT"), 1.0)
	...
	if Input.is_action_just_pressed("build_skypump"):
		_queued_build_skypump = true
...
func _send_input() -> void:
	...
	var cmd := {
		...
		"build_skypump": _queued_build_skypump,
...
func c_uplink_move_ship(_slot: int, _sx: int, _sy: int) -> void:
	pass

@rpc("any_peer", "call_remote", "reliable")
func c_uplink_request_ship(_slot: int, _sx: int, _sy: int) -> void:
	pass
	...
	for e in events:
		...
		if t == "shot":
			...
			if hud != null and hud.has_method("flash_message"):
				hud.call("flash_message", "Ship moved to sector %d,%d" % [int(d.get("sx", 0)), int(d.get("sy", 0))], 1.2)
		elif t == "ship_warp":
			if hud != null and hud.has_method("flash_message"):
				hud.call("flash_message", "Ship warped into sector %d,%d" % [int(d.get("sx", 0)), int(d.get("sy", 0))], 1.2)
		elif t == "ship_reject":
			var who3: int = int(d.get("id", -1))
			if who3 == my_id and hud != null and hud.has_method("flash_message"):
				var reason3: String = str(d.get("reason", "BLOCKED"))
				hud.call("flash_message", "Ship command blocked: %s" % reason3, 1.4)
		elif t == "cargo_spend":
...
func _load_map() -> void:
	...
	# Expose map extents to the HUD for Wulfram-like sector labels.
	var meta: WulframMapMeta = map_instance.get_node_or_null("TerrainRoot/MapMeta") as WulframMapMeta
	if meta != null and is_instance_valid(hud) and hud.has_method("set_world_meta"):
		hud.call("set_world_meta", float(meta.world_width), float(meta.world_depth))
	_fix_terrain_collision(map_instance)
	# Build CPU strategic map textures (Visual/Altitude/Slope) for the M-map overlay.
	_build_strategic_map_textures(map_instance)
	_rebuild_terrain_mesh(map_instance)
	_apply_terrain_visuals(map_instance)


func _build_strategic_map_textures(map_root: Node) -> void:
	# Creates low-res CPU textures for the Strategic Map overlay modes.
	# This uses the terrain heightfield collision data (HeightMapShape3D) so it stays in sync
	# with gameplay collision, and is fast enough to compute once per map load.
	if hud == null or not is_instance_valid(hud) or not hud.has_method("set_stratmap_textures"):
		return
	...
	if meta == null or body == null or col == null:
		return
	...
	# Convert to world heights.
	var heights: PackedFloat32Array = PackedFloat32Array()
	heights.resize(data.size())
	var min_h: float = 1e18
	var max_h: float = -1e18
	for i in range(data.size()):
		var h: float = data[i] * sy
		heights[i] = h
		min_h = minf(min_h, h)
		max_h = maxf(max_h, h)
	...
	# Target texture size. Keep it lightweight but readable.
	var tw: int = 256
	var th: int = 256
	# Respect aspect ratio when clearly non-square.
	var aspect: float = world_d / maxf(world_w, 0.0001)
	th = int(clamp(round(float(tw) * aspect), 160.0, 320.0))

	var img_visual: Image = Image.create(tw, th, false, Image.FORMAT_RGBA8)
	var img_alt: Image = Image.create(tw, th, false, Image.FORMAT_RGBA8)
	var img_slope: Image = Image.create(tw, th, false, Image.FORMAT_RGBA8)

	var c_low := Color(0.08, 0.18, 0.10, 1.0)
	var c_mid := Color(0.12, 0.32, 0.16, 1.0)
	var c_hi := Color(0.45, 0.42, 0.22, 1.0)
	var c_top := Color(0.80, 0.80, 0.78, 1.0)
	var c_rock := Color(0.55, 0.55, 0.58, 1.0)
	var c_slope_lo := Color(0.10, 0.14, 0.22, 1.0)
	var c_slope_hi := Color(0.80, 0.90, 1.00, 1.0)

	for y in range(th):
		var v: float = float(y) / maxf(float(th - 1), 1.0)
		var iz: int = int(floor(v * float(grid_d - 1)))
		for x in range(tw):
			var u: float = float(x) / maxf(float(tw - 1), 1.0)
			var ix: int = int(floor(u * float(grid_w - 1)))
			var idx: int = iz * grid_w + ix
			var h: float = heights[idx]
			var ht: float = clampf((h - min_h) / (max_h - min_h), 0.0, 1.0)
			# Altitude palette
			var alt_col: Color
			if ht < 0.33:
				alt_col = _lerp_color(c_low, c_mid, ht / 0.33)
			elif ht < 0.66:
				alt_col = _lerp_color(c_mid, c_hi, (ht - 0.33) / 0.33)
			else:
				alt_col = _lerp_color(c_hi, c_top, (ht - 0.66) / 0.34)

			# Local slope (use heightfield gradient)
			var xl: int = max(ix - 1, 0)
			var xr: int = min(ix + 1, grid_w - 1)
			var zd: int = max(iz - 1, 0)
			var zu: int = min(iz + 1, grid_d - 1)
			var hl: float = heights[iz * grid_w + xl]
			var hr: float = heights[iz * grid_w + xr]
			var hd: float = heights[zd * grid_w + ix]
			var hu: float = heights[zu * grid_w + ix]
			var sx: float = (hr - hl) / maxf(2.0 * dx, 0.0001)
			var sz: float = (hu - hd) / maxf(2.0 * dz, 0.0001)
			var grad: float = sqrt(sx * sx + sz * sz)
			var ang: float = atan(grad)
			var st: float = clampf(ang / deg_to_rad(45.0), 0.0, 1.0)
			var slope_col: Color = _lerp_color(c_slope_lo, c_slope_hi, st)

			# Visual mode: grassy by height + rocks by slope.
			var vis_base: Color
			if ht < 0.5:
				vis_base = _lerp_color(c_low, c_mid, ht / 0.5)
			else:
				vis_base = _lerp_color(c_mid, c_hi, (ht - 0.5) / 0.5)
			var vis_col: Color = _lerp_color(vis_base, c_rock, st)

			img_visual.set_pixel(x, y, vis_col)
			img_alt.set_pixel(x, y, alt_col)
			img_slope.set_pixel(x, y, slope_col)

	var tex_visual: ImageTexture = ImageTexture.create_from_image(img_visual)
	var tex_alt: ImageTexture = ImageTexture.create_from_image(img_alt)
	var tex_slope: ImageTexture = ImageTexture.create_from_image(img_slope)
	hud.call("set_stratmap_textures", tex_visual, tex_alt, tex_slope)
...
func _ensure_input_actions() -> void:
	...
	if not InputMap.has_action("build_skypump"):
		InputMap.add_action("build_skypump")
		var y := InputEventKey.new()
		y.physical_keycode = KEY_Y
		InputMap.action_add_event("build_skypump", y)
	...
	if not InputMap.has_action("toggle_radar_range"):
		InputMap.add_action("toggle_radar_range")
		var f3r := InputEventKey.new()
		f3r.physical_keycode = KEY_F3
		InputMap.action_add_event("toggle_radar_range", f3r)


	if not InputMap.has_action("toggle_shape_debug"):
		InputMap.add_action("toggle_shape_debug")
		var f3 := InputEventKey.new()
		f3.physical_keycode = KEY_F9
...
func _maybe_add_uplink_base_area(node: Node3D, team: int) -> void:
	if node == null:
		return
	# Prevent duplicates
	if node.get_node_or_null("UplinkBaseArea") != null:
		return

	var area := Node3D.new()
	area.name = "UplinkBaseArea"
	node.add_child(area)

	var team_col: Color = (Color(1.0, 0.25, 0.25, 1.0) if team == 0 else Color(0.2, 0.8, 1.0, 1.0))
	...
	pmat.albedo_color = Color(0.05, 0.05, 0.05, 0.55)
	...
	rmat.albedo_color = Color(team_col.r, team_col.g, team_col.b, 0.45)
	...
	for i in range(2):
		...
		bar.name = "PadBar%d" % i
		var bm := BoxMesh.new()
		bm.size = Vector3(7.4, 0.03, 0.18)
		bar.mesh = bm
		var bmat := StandardMaterial3D.new()
		bmat.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
		bmat.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
		bmat.albedo_color = Color(team_col.r, team_col.g, team_col.b, 0.28)
		bar.material_override = bmat
		bar.position = Vector3(0, 0.03, 0)
		if i == 1:
			bar.rotation_degrees = Vector3(0, 90, 0)
		area.add_child(bar)

	# Perimeter pylons (subtle beacons)
	var offs: Array = [
		Vector3(-3.45, 0, -3.45),
		Vector3(3.45, 0, -3.45),
		Vector3(-3.45, 0, 3.45),
	...
	for j in range(offs.size()):
		...
		pm2.albedo_color = Color(0.15, 0.15, 0.15, 1.0)
		p.material_override = pm2
		p.position = offs[j] + Vector3(0, 0.45, 0)
		area.add_child(p)
		...
		lmat.albedo_color = Color(team_col.r, team_col.g, team_col.b, 0.85)
...
func _shape_for_building(btype: String, team_id: int) -> String:
	...
	if t == "skypump" or t.begins_with("skypump"):
		return "skypump_1" if team_id == 0 else "skypump_2"
	...
	for b in arr:
		...
		if not _buildings.has(bid):
			...
			if bt == "turret" or bt.ends_with("turret"):
				...
				cm.height = 2.2
			elif bt == "uplink" or bt.ends_with("uplink"):
				cm.top_radius = 1.25
				cm.bottom_radius = 1.45
				cm.height = 2.8
			else:
				cm.top_radius = 1.0
			...
			body.position = Vector3(0, 1.1, 0) if (bt == "turret" or bt.ends_with("turret")) else (Vector3(0, 1.4, 0) if (bt == "uplink" or bt.ends_with("uplink")) else Vector3(0, 0.9, 0))
			n.add_child(body)
			...
			# Uplink base area dressing (landing pad + beacons)
			if bt == "uplink" or bt.ends_with("uplink"):
				_maybe_add_uplink_base_area(n, team)
...
func _update_status_from_snapshot(snap: Dictionary) -> void:
	...
	if is_instance_valid(hud):
		...
		if hud.has_method("set_minimap"):
			...
			var ships: Array = []
			if snap.has("_ships") and typeof(snap["_ships"]) == TYPE_ARRAY:
				for se in (snap["_ships"] as Array):
					if typeof(se) != TYPE_DICTIONARY: continue
					var sd: Dictionary = se
					ships.append({
						"id": int(sd.get("id", -1)),
			...
			hud.call("set_minimap", pos, _pred_yaw, team, players, crates, bld, _target_peer_id, ships)
