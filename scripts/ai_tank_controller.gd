...
func _physics_process(delta):
	...
	update_timer += delta
	if update_timer > 0.2: # Think every 0.2s
		update_timer = 0
		decide_logic()
...
func decide_logic():
	...
	match state:
		...
		State.ATTACK:
			# Strafe logic: Try to circle? For now, just stop and shoot
...
func steer_towards(pos):
	...
	if local_target.x > 2.0:
		input_turn = -1.0 # Turn Left
	elif local_target.x < -2.0:
		input_turn = 1.0 # Turn Right
	else:
		input_turn = 0.0
