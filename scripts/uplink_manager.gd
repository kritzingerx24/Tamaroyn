...
func _process(delta):
	...
	if is_menu_open:
		if Input.is_action_just_pressed("select_weapon_1"): call_drop(0)
		elif Input.is_action_just_pressed("select_weapon_2"): call_drop(1)
func call_drop(idx):
	if idx < buildings.size():
		var pt = tank.get_aim_point()
		var ship = dropship_scene.instantiate()
		get_tree().root.add_child(ship)
		ship.setup_delivery(pt, buildings[idx])
		is_menu_open = false
